                        ...
                        @foreach($cart_datas as $cart_data)
                            ...
                            <tr>
                                ...
                                <td class="cart_description">
                                    ...
                                    <p>{{$cart_data->id}} | {{$cart_data->weight}}</p>
                                </td>
                                <td class="cart_price">
                                    <p>NPR {{$cart_data->price}}</p>
                                </td>
                                ...
                                <td class="cart_total">
                                    <p class="cart_total_price">NPR {{$cart_data->price*$cart_data->quantity}}</p>
                                </td>
                    ...
                    <div class="total_area">
                        <ul>
                            @if(Session::has('discount_amount_price'))
                                <li>Sub Total <span>NPR {{$total_price}}</span></li>
                                <li>Coupon Discount (Code : {{Session::get('coupon_code')}}) <span>NPR {{Session::get('discount_amount_price')}}</span></li>
                                <li>Total <span>NPR {{$total_price-Session::get('discount_amount_price')}}</span></li>
                            @else
                                <li>Total <span>NPR {{$total_price}}</span></li>
                            @endif
                        </ul>
                        <div style="margin-left: 20px;"><a class="btn btn-default check_out {{$menu_active==6? ' active':''}}" href="{{url('/check-out')}}">Check Out</a></div>
                    </div>
                </div>
            </div>
        </div>
