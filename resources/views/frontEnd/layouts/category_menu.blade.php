                ...
                <div class="panel-heading">
                    <h4 class="panel-title">
                        <a data-toggle="collapse" data-parent="#accordian" href="#sportswear{{$category->id}}">
                            ...
                            <a href="{{route('cats',$category->id)}}" class="{{$menu_active==1? ' active':''}}">{{$category->name}}</a>
                        ...
                        <div class="panel-body">
                            <ul>
                                @foreach($sub_categories as $sub_category)
                                    <li><a href="{{route('cats',$sub_category->id)}}" class="{{$menu_active==1? ' active':''}}">{{$sub_category->name}} </a></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
